...
public class ActionBarSherlockCompat extends ActionBarSherlock implements MenuBuilder.Callback, com.actionbarsherlock.view.Window.Callback, MenuPresenter.Callback, android.view.MenuItem.OnMenuItemClickListener {
    ...
    public ActionMode startActionMode(ActionMode.Callback callback) {
        ...
        if (mode != null) {
            ...
            if (mActionModeView == null) {
                ViewStub stub = (ViewStub)mDecor.findViewById(CT_Traffic_R.id.abs__action_mode_bar_stub());
                if (stub != null) {
    ...
    private boolean initializePanelMenu() {
        ...
        if (wActionBar != null) {
            ...
            currentTheme.resolveAttribute(CT_Traffic_R.attr.actionBarWidgetTheme(),
                    outValue, true);
    ...
    private IcsProgressBar getCircularProgressBar(boolean shouldInstallDecor) {
        ...
        mCircularProgressBar = (IcsProgressBar)mDecor.findViewById(CT_Traffic_R.id.abs__progress_circular());
        if (mCircularProgressBar != null) {
    ...
    private IcsProgressBar getHorizontalProgressBar(boolean shouldInstallDecor) {
        ...
        mHorizontalProgressBar = (IcsProgressBar)mDecor.findViewById(CT_Traffic_R.id.abs__progress_horizontal());
        if (mHorizontalProgressBar != null) {
    ...
    private void installDecor() {
        ...
        if (mContentParent == null) {
            ...
            if (mTitleView != null) {
                ...
                wActionBar = (ActionBarView)mDecor.findViewById(CT_Traffic_R.id.abs__action_bar());
                if (wActionBar != null) {
                    ...
                    if (splitWhenNarrow) {
                        splitActionBar = getResources_getBoolean(mActivity, CT_Traffic_R.bool.abs__split_action_bar_is_narrow());
                    } else {
                        splitActionBar = mActivity.getTheme()
                                .obtainStyledAttributes(CT_Traffic_R.styleable.SherlockTheme())
                                .getBoolean(CT_Traffic_R.styleable.SherlockTheme_windowSplitActionBar(), false);
                    }
                    final ActionBarContainer splitView = (ActionBarContainer)mDecor.findViewById(CT_Traffic_R.id.abs__split_action_bar());
                    if (splitView != null) {
                        ...
                        mActionModeView = (ActionBarContextView)mDecor.findViewById(CT_Traffic_R.id.abs__action_context_bar());
    ...
    private ViewGroup generateLayout() {
        ...
        TypedArray a = mActivity.getTheme().obtainStyledAttributes(CT_Traffic_R.styleable.SherlockTheme());

        mIsFloating = a.getBoolean(CT_Traffic_R.styleable.SherlockTheme_android_windowIsFloating(), false);

        if (!a.hasValue(CT_Traffic_R.styleable.SherlockTheme_windowActionBar())) {
            throw new IllegalStateException("You must use Theme.Sherlock, Theme.Sherlock.Light, Theme.Sherlock.Light.DarkActionBar, or a derivative.");
        }

        if (a.getBoolean(CT_Traffic_R.styleable.SherlockTheme_windowNoTitle(), false)) {
            requestFeature(Window.FEATURE_NO_TITLE);
        } else if (a.getBoolean(CT_Traffic_R.styleable.SherlockTheme_windowActionBar(), false)) {
            // Don't allow an action bar if there is no title.
            requestFeature(Window.FEATURE_ACTION_BAR);
        }

        if (a.getBoolean(CT_Traffic_R.styleable.SherlockTheme_windowActionBarOverlay(), false)) {
            requestFeature(Window.FEATURE_ACTION_BAR_OVERLAY);
        }

        if (a.getBoolean(CT_Traffic_R.styleable.SherlockTheme_windowActionModeOverlay(), false)) {
        ...
        if (!hasFeature(Window.FEATURE_NO_TITLE)) {
            if (mIsFloating) {
                ...
                layoutResource = CT_Traffic_R.layout.abs__dialog_title_holo();
            } else {
                if (hasFeature(Window.FEATURE_ACTION_BAR_OVERLAY)) {
                    layoutResource = CT_Traffic_R.layout.abs__screen_action_bar_overlay();
                } else {
                    layoutResource = CT_Traffic_R.layout.abs__screen_action_bar();
                }
            }
        } else if (hasFeature(Window.FEATURE_ACTION_MODE_OVERLAY) && !hasFeature(Window.FEATURE_NO_TITLE)) {
            layoutResource = CT_Traffic_R.layout.abs__screen_simple_overlay_action_mode();
        } else {
            layoutResource = CT_Traffic_R.layout.abs__screen_simple();
        }
        ...
        ViewGroup contentParent = (ViewGroup)mDecor.findViewById(CT_Traffic_R.id.abs__content());
