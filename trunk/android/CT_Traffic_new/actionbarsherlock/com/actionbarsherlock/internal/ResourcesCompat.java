...
public final class ResourcesCompat {
    ...
    public static boolean getResources_getBoolean(Context context, int id) {
        ...
        if (id == ResUtil.getInstance(context).getBool("abs__action_bar_embed_tabs")/* CT_Traffic_R.bool.abs__action_bar_embed_tabs()*/) {
            if (widthDp >= 480) {
                return true; //values-w480dp
            }
            return false; //values
        }
        if (id == ResUtil.getInstance(context).getBool("abs__split_action_bar_is_narrow")/*CT_Traffic_R.bool.abs__split_action_bar_is_narrow()*/) {
            if (widthDp >= 480) {
                return false; //values-w480dp
            }
            return true; //values
        }
        if (id == ResUtil.getInstance(context).getBool("abs__action_bar_expanded_action_views_exclusive")/*CT_Traffic_R.bool.abs__action_bar_expanded_action_views_exclusive()*/) {
            if (smallestWidthDp >= 600) {
                return false; //values-sw600dp
            }
            return true; //values
        }
        if (id == ResUtil.getInstance(context).getBool("abs__config_allowActionMenuItemTextWithIcon")/*CT_Traffic_R.bool.abs__config_allowActionMenuItemTextWithIcon()*/) {
            if (widthDp >= 480) {
                return true; //values-w480dp
            }
            return false; //values
    ...
    public static int getResources_getInteger(Context context, int id) {
        ...
        if (id == ResUtil.getInstance(context).getInteger("abs__max_action_buttons")/* CT_Traffic_R.integer.abs__max_action_buttons()*/) {
            if (widthDp >= 600) {
