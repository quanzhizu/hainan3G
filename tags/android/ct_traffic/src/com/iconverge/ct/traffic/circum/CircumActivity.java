...
public class CircumActivity extends BaseActivity{
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(R.layout.circum);
...
//		}
		...
		tvMyLocation = (TextView) findViewById(R.id.circum_tv_mylocation_value);
		rgDistance = (RadioGroup) findViewById(R.id.circum_rg_distance);
		llProgressBar = (LinearLayout) findViewById(R.id.circum_ll_progressbar);
		lvResult = (ListView) findViewById(R.id.circum_lv_result);
		...
		rgDistance.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				if (checkedId == R.id.circum_rb_500) {
					intDistance = 500;
				} else if (checkedId == R.id.circum_rb_1000) {
					intDistance = 1000;
				} else if (checkedId == R.id.circum_rb_3000) {
					intDistance = 3000;
				} else if (checkedId == R.id.circum_rb_5000) {
					intDistance = 5000;
				}
...
//				showToast(item.getTitle() + "\n" + getLocationByGeoPoint(item.getPoint(), 1) + "\n" + item.getTypeCode() + ":" + item.getTypeDes());
	...
	class CircumFoodTask extends AsyncTask<Void, Void, List<PoiItem>> {
		...
		protected void onPostExecute(List<PoiItem> result) {
			...
			actionBar.setSubtitle(MessageFormat.format(getString(R.string.total_of), result.size()));
