...
public class HotspotSearchResultActivity extends BaseActivity implements View.OnClickListener {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		context = this;
		setContentView(ResUtil.getInstance(context).getLayout("ct_traffic__hotspot_search_result"));
		...
		llProgressBar = (LinearLayout) findViewById(ResUtil.getInstance(context).getId("hotspot_search_result_ll_progressbar"));
		btnDistance = (Button) findViewById(ResUtil.getInstance(context).getId("hotspot_search_result_btn_distance"));
		btnType = (Button) findViewById(ResUtil.getInstance(context).getId("hotspot_search_result_btn_type"));
		btnOrder = (Button) findViewById(ResUtil.getInstance(context).getId("hotspot_search_result_btn_order"));
		lvResult = (ListView) findViewById(ResUtil.getInstance(context).getId("hotspot_search_result_lv_result"));
	...
	public void onClick(View v) {
		if(v.getId() == ResUtil.getInstance(context).getId("hotspot_search_result_btn_distance")){
			if (!popupWindowDistances.isShowing()) {
				popupWindowDistances.showAsDropDown(btnDistance, btnDistance.getLeft(), 0);
			}
		}else if(v.getId() == ResUtil.getInstance(context).getId("hotspot_search_result_btn_type")){
			if (!popupWindowTypes.isShowing()) {
				popupWindowTypes.showAsDropDown(btnType, 0, 0);
			}
		}else if(v.getId() == ResUtil.getInstance(context).getId("hotspot_search_result_btn_order")){
			showToast(btnOrder.getText().toString());
		}
	}

	private void initPopupWindowDistance() {
		View viewPopupWindow = (View) this.getLayoutInflater().inflate(ResUtil.getInstance(context).getLayout("ct_traffic__hotspot_popup_types"), null);
		ListView listView = (ListView) viewPopupWindow.findViewById(ResUtil.getInstance(context).getId("hotspot_popup_types_lv_types"));
	...
	private void initPopuWindowTypes() {
		View popupWindow = (View) this.getLayoutInflater().inflate(ResUtil.getInstance(context).getLayout("ct_traffic__hotspot_popup_types"), null);
		ListView listView = (ListView) popupWindow.findViewById(ResUtil.getInstance(context).getId("hotspot_popup_types_lv_types"));
		listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
