...
public class StationActivity extends BaseActivity {
	...
	private String city = "500000";//"330100";//城市代码 500000 重庆
	...
	protected void onCreate(Bundle arg0) {
		...
		String url = "http://116.228.55.155:6060/dataquery/query?sid=102&key=" + getString(R.string.maps_api_key) + "&city="+city+"&srctype=BUS&range=1500&page=1&rows=10&cenx=" + Const.longitude + "&ceny=" + Const.latitude + "&restype=json";
		// #debug debug
		System.out.println("url == " + url);
		sAdapt = new StationAdapt(getApplicationContext());
		listView = (ListView) findViewById(R.id.busstation_lv);
		listView.setAdapter(sAdapt);
		listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {

			@Override
			public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
				StationBean bean = (StationBean) arg0.getItemAtPosition(position);
				if (bean != null) {
					...
					if (str != null && str.length == 2) {
						...
						PoiItem poiItem = new PoiItem(PoiTypeDef.PublicTransportation, geoPoint, bean.getName(), bean.getAddr());
						poiItems[0] = poiItem;
...
//		showProgressBar(getString(R.string.query_ing));
			...
			public void textLoaded(String text) {
				if (text != null) {
					try {
						...
						Type listType = new TypeToken<List<StationBean>>() {
						}.getType();
						Gson gson = new Gson();
						List<StationBean> station = gson.fromJson(jObject1.getString("docs"), listType);
	...
	public List<StationBean> sortStationBean(List<StationBean> lists) {
		if (lists != null && !lists.isEmpty()) {
			Collections.sort(lists, new Comparator<StationBean>() {
				@Override
				public int compare(StationBean c1, StationBean c2) {
...
//		showToast(getString(R.string.query_error));
		...
		noDataTextView.setText(getString(R.string.query_error_busstation));
	}
}
