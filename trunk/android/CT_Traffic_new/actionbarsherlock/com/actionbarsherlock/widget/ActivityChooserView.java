...
class ActivityChooserView extends ViewGroup implements ActivityChooserModelClient {
    ...
    public ActivityChooserView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray attributesArray = context.obtainStyledAttributes(attrs,
                ResUtil.getInstance(context).getStyleable("SherlockActivityChooserView", false)/*CT_Traffic_R.styleable.SherlockActivityChooserView()*/, defStyle, 0);

        mInitialActivityCount = attributesArray.getInt(
        		ResUtil.getInstance(context).getStyleable("SherlockActivityChooserView_initialActivityCount")/*CT_Traffic_R.styleable.SherlockActivityChooserView_initialActivityCount()*/,
                ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_DEFAULT);

        Drawable expandActivityOverflowButtonDrawable = attributesArray.getDrawable(
        		ResUtil.getInstance(context).getStyleable("SherlockActivityChooserView_expandActivityOverflowButtonDrawable")/*CT_Traffic_R.styleable.SherlockActivityChooserView_expandActivityOverflowButtonDrawable()*/);

        attributesArray.recycle();

        LayoutInflater inflater = LayoutInflater.from(mContext);
        inflater.inflate(ResUtil.getInstance(context).getLayout("abs__activity_chooser_view")/* CT_Traffic_R.layout.abs__activity_chooser_view()*/, this, true);

        mCallbacks = new Callbacks();

        mActivityChooserContent = (IcsLinearLayout) findViewById(ResUtil.getInstance(context).getId("abs__activity_chooser_view_content")/* CT_Traffic_R.id.abs__activity_chooser_view_content()*/);
        mActivityChooserContentBackground = mActivityChooserContent.getBackground();

        mDefaultActivityButton = (FrameLayout) findViewById(ResUtil.getInstance(context).getId("abs__default_activity_button")/*CT_Traffic_R.id.abs__default_activity_button()*/);
        mDefaultActivityButton.setOnClickListener(mCallbacks);
        mDefaultActivityButton.setOnLongClickListener(mCallbacks);
        mDefaultActivityButtonImage = (ImageView) mDefaultActivityButton.findViewById(ResUtil.getInstance(context).getId("abs__image")/*CT_Traffic_R.id.abs__image()*/);

        mExpandActivityOverflowButton = (FrameLayout) findViewById(ResUtil.getInstance(context).getId("abs__expand_activities_button")/*CT_Traffic_R.id.abs__expand_activities_button()*/);
        mExpandActivityOverflowButton.setOnClickListener(mCallbacks);
        mExpandActivityOverflowButtonImage =
            (ImageView) mExpandActivityOverflowButton.findViewById(ResUtil.getInstance(context).getId("abs__image")/*CT_Traffic_R.id.abs__image()*/);
        ...
        mListPopupMaxWidth = Math.max(resources.getDisplayMetrics().widthPixels / 2,
              resources.getDimensionPixelSize(ResUtil.getInstance(mContext).getDimen("abs__config_prefDialogWidth")/* CT_Traffic_R.dimen.abs__config_prefDialogWidth()*/));
    }
    ...
    private void showPopupUnchecked(int maxActivityCount) {
        ...
        if (!popupWindow.isShowing()) {
            ...
            popupWindow.getListView().setContentDescription(mContext.getString(
                    ResUtil.getInstance(mContext).getString("abs__activitychooserview_choose_application")/* CT_Traffic_R.string.abs__activitychooserview_choose_application()*/));
        }
    }
                ...
                case ITEM_VIEW_TYPE_FOOTER:
                    if (convertView == null || convertView.getId() != ITEM_VIEW_TYPE_FOOTER) {
                        convertView = LayoutInflater.from(getContext()).inflate(
                                ResUtil.getInstance(mContext).getLayout("abs__activity_chooser_view_list_item")/* CT_Traffic_R.layout.abs__activity_chooser_view_list_item()*/, parent, false);
                        convertView.setId(ITEM_VIEW_TYPE_FOOTER);
                        TextView titleView = (TextView) convertView.findViewById(ResUtil.getInstance(mContext).getId("abs__title")/* CT_Traffic_R.id.abs__title()*/);
                        titleView.setText(mContext.getString(
                        		ResUtil.getInstance(mContext).getString("abs__activity_chooser_view_see_all")/*CT_Traffic_R.string.abs__activity_chooser_view_see_all()*/));
                    }
                    return convertView;
                case ITEM_VIEW_TYPE_ACTIVITY:
                    if (convertView == null || convertView.getId() != ResUtil.getInstance(mContext).getId("abs__list_item")/*CT_Traffic_R.id.abs__list_item()*/) {
                        convertView = LayoutInflater.from(getContext()).inflate(
                        		ResUtil.getInstance(mContext).getLayout("abs__activity_chooser_view_list_item")/*CT_Traffic_R.layout.abs__activity_chooser_view_list_item()*/, parent, false);
                    }
                    PackageManager packageManager = mContext.getPackageManager();
                    // Set the icon
                    ImageView iconView = (ImageView) convertView.findViewById(ResUtil.getInstance(mContext).getId("abs__icon")/* CT_Traffic_R.id.abs__icon()*/);
