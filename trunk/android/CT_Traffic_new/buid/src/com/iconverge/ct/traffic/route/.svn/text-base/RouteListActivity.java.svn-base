...
public class RouteListActivity extends BaseActivity {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		this.context = this;
		setContentView(ResUtil.getInstance(context).getLayout("ct_traffic__route_list"));
		...
		if (intent == null) {
			...
			Route route = DataManager.getInstance().routes.get(0);
			startText = (TextView) findViewById(ResUtil.getInstance(context).getId("from_add"));
			startText.setText(intent.getStringExtra("start"));
			endText = (TextView) findViewById(ResUtil.getInstance(context).getId("to_add"));
			endText.setText(intent.getStringExtra("end"));
			TextView info = (TextView) findViewById(ResUtil.getInstance(context).getId("route_info"));
			StringBuilder builder = new StringBuilder(getString(ResUtil.getInstance(context).getString("ct_traffic__route_")));
			builder.append("  ");
			builder.append(MessageFormat.format(getString(ResUtil.getInstance(context).getString("ct_traffic___meter")), route.getLength()));
			...
			ListView listView = (ListView) findViewById(ResUtil.getInstance(context).getId("list"));
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		menu.add(0, 1, 0, getResources().getString(ResUtil.getInstance(context).getString("ct_traffic__route_on_map"))).setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM | MenuItem.SHOW_AS_ACTION_WITH_TEXT);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case 1:
			showProgressBar(getString(ResUtil.getInstance(context).getString("ct_traffic__get_rtsp")));
			getCameraPoi();
			break;
