...
public class SearchView extends LinearLayout implements CollapsibleActionView {
    ...
    */
    ...
    public SearchView(Context context, AttributeSet attrs) {
        ...
        inflater.inflate(CT_Traffic_R.layout.abs__search_view(), this, true);

        mSearchButton = findViewById(CT_Traffic_R.id.abs__search_button());
        mQueryTextView = (SearchAutoComplete) findViewById(CT_Traffic_R.id.abs__search_src_text());
        mQueryTextView.setSearchView(this);

        mSearchEditFrame = findViewById(CT_Traffic_R.id.abs__search_edit_frame());
        mSearchPlate = findViewById(CT_Traffic_R.id.abs__search_plate());
        mSubmitArea = findViewById(CT_Traffic_R.id.abs__submit_area());
        mSubmitButton = findViewById(CT_Traffic_R.id.abs__search_go_btn());
        mCloseButton = (ImageView) findViewById(CT_Traffic_R.id.abs__search_close_btn());
        mVoiceButton = findViewById(CT_Traffic_R.id.abs__search_voice_btn());
        mSearchHintIcon = (ImageView) findViewById(CT_Traffic_R.id.abs__search_mag_icon());
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, CT_Traffic_R.styleable.SherlockSearchView(), 0, 0);
        setIconifiedByDefault(a.getBoolean(CT_Traffic_R.styleable.SherlockSearchView_iconifiedByDefault(), true));
        int maxWidth = a.getDimensionPixelSize(CT_Traffic_R.styleable.SherlockSearchView_android_maxWidth(), -1);
        if (maxWidth != -1) {
            setMaxWidth(maxWidth);
        }
        CharSequence queryHint = a.getText(CT_Traffic_R.styleable.SherlockSearchView_queryHint());
        if (!TextUtils.isEmpty(queryHint)) {
            setQueryHint(queryHint);
        }
        int imeOptions = a.getInt(CT_Traffic_R.styleable.SherlockSearchView_android_imeOptions(), -1);
        if (imeOptions != -1) {
            setImeOptions(imeOptions);
        }
        int inputType = a.getInt(CT_Traffic_R.styleable.SherlockSearchView_android_inputType(), -1);
        if (inputType != -1) {
        ...
        a = context.obtainStyledAttributes(attrs, CT_Traffic_R.styleable.SherlockView(), 0, 0);
        focusable = a.getBoolean(CT_Traffic_R.styleable.SherlockView_android_focusable(), focusable);
        a.recycle();
    ...
    private int getPreferredWidth() {
        return getContext().getResources()
                .getDimensionPixelSize(CT_Traffic_R.dimen.abs__search_view_preferred_width());
    }
    ...
    private int getSearchIconId() {
        ...
        getContext().getTheme().resolveAttribute(CT_Traffic_R.attr.searchViewSearchIcon(),
                outValue, true);
    ...
    private void adjustDropDownSizeAndPosition() {
        if (mDropDownAnchor.getWidth() > 1) {
            ...
            int iconOffset = mIconifiedByDefault
                    ? res.getDimensionPixelSize(CT_Traffic_R.dimen.abs__dropdownitem_icon_width())
                    + res.getDimensionPixelSize(CT_Traffic_R.dimen.abs__dropdownitem_text_padding_left())
                    : 0;
