...
public class ResUtil {
	...
	private ResUtil(Context context) {
		...
		try {
			integer = Class.forName(this.context.getPackageName() + ".R$integer");
		} catch (ClassNotFoundException e) {
			Log.d(TAG, e.getMessage());
		}
		try {
	...
	public int[] getStyleable(String paramString,boolean flag) {
		return getResOfR(styleable, paramString,flag);
	}

	public int getInteger(String paramString) {
		return getResOfR(integer, paramString);
	}
	...
	private int[] getResOfR(Class<?> paramClass, String paramString,boolean flag) {
		if (paramClass == null) {
			Log.d(TAG, "getRes(null," + paramString + ")");
			throw new IllegalArgumentException("ResClass is not initialized.");
		}
		try {
			Field localField = paramClass.getField(paramString);
			Object o = localField.get(paramString);
			if(o instanceof int[]){
				return (int[])o;
			}
		} catch (Exception localException) {
