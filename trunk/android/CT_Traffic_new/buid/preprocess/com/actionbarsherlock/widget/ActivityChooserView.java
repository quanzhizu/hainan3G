...
class ActivityChooserView extends ViewGroup implements ActivityChooserModelClient {
    ...
    public ActivityChooserView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray attributesArray = context.obtainStyledAttributes(attrs,
                CT_Traffic_R.styleable.SherlockActivityChooserView(), defStyle, 0);

        mInitialActivityCount = attributesArray.getInt(
        		CT_Traffic_R.styleable.SherlockActivityChooserView_initialActivityCount(),
                ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_DEFAULT);

        Drawable expandActivityOverflowButtonDrawable = attributesArray.getDrawable(
        		CT_Traffic_R.styleable.SherlockActivityChooserView_expandActivityOverflowButtonDrawable());

        attributesArray.recycle();

        LayoutInflater inflater = LayoutInflater.from(mContext);
        inflater.inflate(CT_Traffic_R.layout.abs__activity_chooser_view(), this, true);

        mCallbacks = new Callbacks();

        mActivityChooserContent = (IcsLinearLayout) findViewById(CT_Traffic_R.id.abs__activity_chooser_view_content());
        mActivityChooserContentBackground = mActivityChooserContent.getBackground();

        mDefaultActivityButton = (FrameLayout) findViewById(CT_Traffic_R.id.abs__default_activity_button());
        mDefaultActivityButton.setOnClickListener(mCallbacks);
        mDefaultActivityButton.setOnLongClickListener(mCallbacks);
        mDefaultActivityButtonImage = (ImageView) mDefaultActivityButton.findViewById(CT_Traffic_R.id.abs__image());

        mExpandActivityOverflowButton = (FrameLayout) findViewById(CT_Traffic_R.id.abs__expand_activities_button());
        mExpandActivityOverflowButton.setOnClickListener(mCallbacks);
        mExpandActivityOverflowButtonImage =
            (ImageView) mExpandActivityOverflowButton.findViewById(CT_Traffic_R.id.abs__image());
        ...
        mListPopupMaxWidth = Math.max(resources.getDisplayMetrics().widthPixels / 2,
              resources.getDimensionPixelSize(CT_Traffic_R.dimen.abs__config_prefDialogWidth()));
    }
    ...
    private void showPopupUnchecked(int maxActivityCount) {
        ...
        if (!popupWindow.isShowing()) {
            ...
            popupWindow.getListView().setContentDescription(mContext.getString(
                    CT_Traffic_R.string.abs__activitychooserview_choose_application()));
        }
    }
                ...
                case ITEM_VIEW_TYPE_FOOTER:
                    if (convertView == null || convertView.getId() != ITEM_VIEW_TYPE_FOOTER) {
                        convertView = LayoutInflater.from(getContext()).inflate(
                                CT_Traffic_R.layout.abs__activity_chooser_view_list_item(), parent, false);
                        convertView.setId(ITEM_VIEW_TYPE_FOOTER);
                        TextView titleView = (TextView) convertView.findViewById(CT_Traffic_R.id.abs__title());
                        titleView.setText(mContext.getString(
                        		CT_Traffic_R.string.abs__activity_chooser_view_see_all()));
                    }
                    return convertView;
                case ITEM_VIEW_TYPE_ACTIVITY:
                    if (convertView == null || convertView.getId() != CT_Traffic_R.id.abs__list_item()) {
                        convertView = LayoutInflater.from(getContext()).inflate(
                        		CT_Traffic_R.layout.abs__activity_chooser_view_list_item(), parent, false);
                    }
                    PackageManager packageManager = mContext.getPackageManager();
                    // Set the icon
                    ImageView iconView = (ImageView) convertView.findViewById(CT_Traffic_R.id.abs__icon());
